...
function StorysShops({
  ...
  const btnAdd = document.querySelector("#task112" + index);
  ...
  function createMarkupTextFO() {
    if (value.fake_content !== "") {
      var newText = value.fake_content.slice(100, 110);
      if (value.fake_content.slice(0, 100).indexOf(newText) === -1) {
        ...
        if (hashText !== -1) {
          ...
          const sellText = value.content.slice(1250, 1350).indexOf(">");
          if (sellText !== -1) {
            return value.content
              .slice(0, value.content.slice(0, 1350).lastIndexOf(">") + 1)
              .concat(
                `<a style="color: #aeafaf; cursor: pointer; text-decoration: none;" id="${
                  "task112" + index
                }">... Xem thêm</a>`
            ...
            const endText = value.content.slice(1250, 1350).indexOf("<");
            if (endText !== -1) {
              return value.content
                .slice(0, value.content.slice(0, 1350).lastIndexOf("<"))
                .concat(
                  `<a style="color: #aeafaf; cursor: pointer; text-decoration: none;" id="${
                    "task112" + index
                  }">... Xem thêm</a>`
      ...
      {value.post_status === "normal" ? (
        <div>
          ...
          >
            ...
            <div style={{fontWeight:"bold", color:"green"}}>{value.money}</div>
          </div>
          {checkNull(value.format) && <div className="homestory-image">
            <Images images={value.format.split(";")} />
          </div>}
